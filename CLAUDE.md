...
- **`pragma!` is real, and `max-stack-depth` is the one it acts on.** `Pragmas` records the setting and answers the unit atom; a bad depth answers `(Error (pragma! …) UnsignedIntegerIsExpected)` as the reference does. A bounded program's stack overflow becomes `(Error <run> StackOverflow)` and therefore ends the program; an unbounded one stays a `StackOverflowError`. **The unit diverges on purpose:** the reference counts minimal-MeTTa instructions (its `max-stack-depth 200` ≈ 6 levels of `fac`), we count MeTTa CALLS, enforced by sizing the thread stack from a literal pragma at compile time — a counter in every prologue is the per-call cost this compiler does not pay.
- **`quote` keeps its wrapper; an `Atom`-result function answers its body as a term.** A source `(quote X)` is the compiler's internal quote (`PredefinedAtoms.QUOTE`, a `Special`) of the WHOLE term, so it answers `(quote X)`; codegen peels only the `Special`, never a `quote` Symbol, and `unquote` is the library's rule. A function declaring its result literally `Atom` compiles its clause body as the quoted source term (`FunctionRewriter.templateAtomResultBodies`) — except a minimal-MeTTa body (`function`/`chain`/`eval`/`return`/`metta`, i.e. every such stdlib function), a multivalued one, or one with a function-typed parameter. So do not use `-> … Atom` as an inference hint in a test that expects the body evaluated: use `%Undefined%`.
- **A top-level `(Error …)` ENDS the program.** Generated `__main` hands every run's result to `Errors.checkRunResult`, which throws `MettaError` when the result (or any element of a multivalued result bag) is an expression headed by `Error` — the reference switches its runner to `MettaRunnerMode::TERMINATE` at exactly that point, so the runs below it never evaluate. The check is shallow, like hyperon's `atom_is_error`: `(foo (Error …))` is ordinary data. The test-runner scores it `ERROR_TERM`, a FAILURE — scoring a program that stopped early as PASS is what would inflate every corpus measurement.
...
- **Reference-semantics probes** live in `tests/hyperon/` (one behaviour per file, answers measured on `metta-repl`; known divergences in `.xfail`). Measure any new probe on the reference first.
